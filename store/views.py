...
def report1(request):
    ...
    writer.writerow([_('Time'),
                     _('Name'),
                     _('Price'),
                     _('Discount'),
                     _('Discount Amount')])
    # data calculations
    data = Sell.objects.filter(
        time_of_purchase__range=[start_date,
                                 start_date + timedelta(days=1)]) \
        .annotate(
        discount_amount=F('product__price') * F('discount__value') / 100) \
        .values_list('time_of_purchase',
                     'product__name',
                     'product__price',
                     'discount__title',
                     'discount_amount')
...
def report2(request):
    ...
    for cat in list(Discount.objects.filter(
            category__isnull=False).values_list('category__category', flat=True)):
...
def report3(request):
    ...
    # time calculations
    # the latest time of purchase
    fm = Sell.objects.aggregate(Max('time_of_purchase'))
    # the earliest time of purchase
    fmin = Sell.objects.aggregate(Min('time_of_purchase'))
    # total days
    delta = (fm['time_of_purchase__max']
             - fmin['time_of_purchase__min']).days
        ...
        if not list(Sell.objects.filter(discount=disc)):
            ...
            for prod in list(Sell.objects.filter(
                ...
                if not list(Sell.objects.filter(product=prod).filter(discount__isnull=True)):
                    ...
                    avg = Sell.objects.filter(product=prod).filter(
                        discount__isnull=True).aggregate(
                        Sum('amount_of_items'))['amount_of_items__sum'] / delta
